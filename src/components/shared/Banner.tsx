    ...
    <section className="relative w-full h-screen bg-[#020202] flex flex-col justify-end md:justify-center overflow-hidden font-sans">
      ...
      <div className="relative z-10 w-full px-6 md:px-16 lg:px-24 flex flex-col md:flex-row justify-between items-center gap-10">
        ...
        <div className="w-full md:max-w-[55%] space-y-4 md:space-y-6 text-left">
          ...
          <div className="md:hidden flex gap-5 overflow-x-auto no-scrollbar pt-8 pb-22 pl-6 pr-6">
            {itens.map((m, idx) => (
              <div
                key={m.id}
        ...
        ${indexAtual === idx
                    ? 'scale-110 ring-2 ring-amber-500 z-20 shadow-[0_0_25px_rgba(245,158,11,0.4)] translate-y-[-12px]'
            ...
            ${indexAtual === idx ? 'text-amber-500 opacity-100' : 'text-white opacity-30'}`}
                  style={{ WebkitTextStroke: indexAtual === idx ? '0px' : '1px rgba(255,255,255,1)', color: indexAtual === idx ? '' : 'transparent' }}>
                  0{idx + 1}
                </span>
              </div>
            ))}
        ...
        <div className="hidden md:flex flex-col gap-4 pl-12 pr-12 border-r border-white/10 max-h-[70vh] overflow-y-auto no-scrollbar py-6">
      ...
      <div className="absolute right-4 lg:right-8 top-1/2 -translate-y-1/2 hidden lg:flex flex-col items-center gap-6 opacity-80">
